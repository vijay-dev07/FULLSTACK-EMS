...
// Send Email to admin , If admin doesn't take an action on leave application within 24 hours
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({event , step}) => {
    ...
    const leaveApplication = await LeaveApplication.find(leaveApplicationId)
...
const attendanceReminderCron = inngest.createFunction(
  { id: "attendance-reminder-cron" ,triggers: [{cron:"TZ=Asia/Kolkata 30 11 * * *"}] }, // 06:00 UTC 11:30 am 
  ...
  async ({step}) => {
    ...
    const today = await step.run("get-today-date" , () => {
        const startUTC = new Date(new Date().toLocaleDateString("en-CA" , {timeZone:"Asia/Kolkata"}) + "T00:00:00+5:30");
        const endUTC = new Date(startUTC.getTime() + 24 * 60 * 60 * 1000);

        return {startUTC: startUTC.toISOString() , endUTC:endUTC.toISOString()}
    })

    // step 2 : get all active , non-deleted employess
    ...
    const checkedInIds = await step.run("get-checked-in-ids" ,
     async () => {
        ...
        return attendances.map((e) => a.employeeId.toString())
    })
